...
| 2026-04-24 | GitHub Copilot (GPT-5.4) | Fix DCC batch entrypoint so SLURM uses the repo runner after `git reset`, ensuring latest ablation/resume logic is actually executed | `notebooks/submit.slurm` | Replaced external training-script path with the repo copy, added a missing-file guard, and passed `--work_dir` explicitly so requeued jobs resume with current code and continue into ablations |
