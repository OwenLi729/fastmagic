...
Tuned issues with the curves, primarily issues with formatting (such as legends being inside of the title) as well as redundacies in plotting the heatmap like the AI plotting beta=3.0 and layers=2 even though the default control already has beta=3.0 and layers=2